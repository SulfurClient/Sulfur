...
import gg.sulfur.client.impl.events.TargetDeathEvent;
...
public class PlayerControllerMP {
    ...
    public boolean func_180511_b(BlockPos p_180511_1_, EnumFacing p_180511_2_) {
        ...
        if (!this.mc.theWorld.getWorldBorder().contains(p_180511_1_)) {
            ...
            if (this.currentGameType.isCreative()) {
                ...
                if (var5 && var3.getPlayerRelativeBlockHardness(this.mc.thePlayer) >= 1.0F) {
    ...
    public boolean func_180512_c(BlockPos p_180512_1_, EnumFacing p_180512_2_) {
        ...
        if (this.blockHitDelay > 0) {
            ...
            if (var3.getMaterial() == Material.air) {
                ...
                this.curBlockDamageMP += var3.getPlayerRelativeBlockHardness(this.mc.thePlayer);
    ...
    public float getBlockReachDistance() {
       // final Nuker nuker = Client.INSTANCE.getModuleManager().get(Nuker.class);
      //  final Teleport teleport = Client.INSTANCE.getModuleManager().get(Teleport.class);
       // if (nuker.isToggled() && nuker.mode.getValue().equals(Nuker.Mode.TELEPORT) || nuker.isToggled() && nuker.mode.getValue().equals(Nuker.Mode.FIRE_TP) && nuker.stopwatch.timeElapsed(50L) || teleport.isToggled() && teleport.mode.getValue().equals(Teleport.Mode.CLICK)) {
      //      return 999.0F;
    //    }
    ...
    public void attackEntity(EntityPlayer playerIn, Entity targetEntity) {
        ...
        if (targetEntity instanceof EntityLivingBase) {
            if (targetEntity.isDead) {
                TargetDeathEvent deathEvent = new TargetDeathEvent(targetEntity);
                Sulfur.getInstance().getEventBus().post(deathEvent);
            }
        }
    ...
    public boolean func_178894_a(EntityPlayer p_178894_1_, Entity p_178894_2_, Vec3 cum) {
        this.syncCurrentPlayItem();
        this.netClientHandler.addToSendQueue(new C02PacketUseEntity(p_178894_2_, cum));
        return this.currentGameType != WorldSettings.GameType.SPECTATOR && p_178894_2_.func_174825_a(p_178894_1_, cum);
    }
    ...
    public boolean onPlayerRightClick(EntityPlayerSP player, WorldClient worldIn, ItemStack heldStack, BlockPos hitPos,
                                      EnumFacing side, Vec3d vec3d) {
        syncCurrentPlayItem();
        float f = (float) (vec3d.xCoord - (double) hitPos.getX());
        float f1 = (float) (vec3d.yCoord - (double) hitPos.getY());
        float f2 = (float) (vec3d.zCoord - (double) hitPos.getZ());
        boolean flag = false;
        ...
        if (!mc.theWorld.getWorldBorder().contains(hitPos)) {
            ...
            netClientHandler.addToSendQueue(new C08PacketPlayerBlockPlacement(hitPos, side.getIndex(),
