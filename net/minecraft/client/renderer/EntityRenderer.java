...
import gg.sulfur.client.impl.modules.movement.InvMove;
import shadersmod.client.Shaders;
import shadersmod.client.ShadersRender;
import gg.sulfur.client.Sulfur;
import gg.sulfur.client.impl.events.Render3DEvent;
import gg.sulfur.client.impl.modules.render.Camera;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float p_78473_1_) {
        ...
        if (var2 != null && this.mc.theWorld != null) {
            ...
            if (pointedEntity != null && (var13 < var5 || this.mc.objectMouseOver == null)) {
                this.mc.objectMouseOver = new MovingObjectPosition(pointedEntity, var10);
    ...
    private void hurtCameraEffect(float p_78482_1_) {
        final Camera camera = Sulfur.getInstance().getModuleManager().get(Camera.class);
        if (camera.isToggled() && camera.noHurtCam.getValue())
            return;
    ...
    private void orientCamera(float p_78467_1_) {
        ...
        /*/ This makes the camera not go up and down /*/
        if (Sulfur.getInstance().getModuleManager().isEnabled(Speed.class)) {
            Speed speed = Sulfur.getInstance().getModuleManager().get(Speed.class);
            if (speed.getMode().getModeName().equalsIgnoreCase("Verus Ground")) {
                mc.thePlayer.posY = Math.round(mc.thePlayer.posY);
            }
        }

        if (Sulfur.getInstance().getModuleManager().isEnabled(Flight.class)) {
            Flight flight = Sulfur.getInstance().getModuleManager().get(Flight.class);
            if (flight.getMode().getModeName().equalsIgnoreCase("Verus")) {
                mc.thePlayer.posY = Math.round(mc.thePlayer.posY);
            }
        }
        ...
        if (var2 instanceof EntityLivingBase && ((EntityLivingBase) var2).isPlayerSleeping()) {
            ...
            if (!this.mc.gameSettings.debugCamEnable) {
                ...
                if (Reflector.ForgeHooksClient_orientBedCamera.exists()) {
                    Reflector.callVoid(Reflector.ForgeHooksClient_orientBedCamera, this.mc.theWorld, var27, partialTicks, var2);
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
            if (!this.mc.gameSettings.debugCamEnable) {
                ...
                Object var33 = Reflector.newInstance(Reflector.EntityViewRenderEvent_CameraSetup_Constructor, this, var2, var32, p_78467_1_, yaw, pitch, var31);
    ...
    private void updateTorchFlicker() {
        ...
        this.torchFlickerX += (this.field_175075_L - this.torchFlickerX);
    ...
    public void updateCameraAndRender(float partialTicks) {
        ...
        InvMove invMove = Sulfur.getInstance().getModuleManager().get(InvMove.class);
        if (this.mc.inGameHasFocus && var2 ||
                (invMove.lookInInv.getValue() && mc.currentScreen != null && mc.currentScreen instanceof GuiChest)) {
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    if (Reflector.ForgeHooksClient_drawScreen.exists()) {
                        Reflector.callVoid(Reflector.ForgeHooksClient_drawScreen, this.mc.currentScreen, var161, var171, partialTicks);
                    } else {
                    ...
                    var11.addCrashSectionCallable("Screen name", () -> EntityRenderer.this.mc.currentScreen.getClass().getCanonicalName());
                    // private static final String __OBFID = "CL_00000950";
                    var11.addCrashSectionCallable("Mouse location", () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d)", var161, var171, Mouse.getX(), Mouse.getY()));
                    // private static final String __OBFID = "CL_00000951";
                    var11.addCrashSectionCallable("Screen size", () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", var131.getScaledWidth(), var131.getScaledHeight(), EntityRenderer.this.mc.displayWidth, EntityRenderer.this.mc.displayHeight, var131.getScaleFactor()));
    ...
    private void func_175068_a(int pass, float partialTicks, long finishTimeNano) {
        ...
        if (!this.field_175078_W) {
            ...
            if (this.mc.objectMouseOver != null && var9.isInsideOfMaterial(Material.water) && var7) {
                ...
                if ((!Reflector.ForgeHooksClient_onDrawBlockHighlight.exists() || !Reflector.callBoolean(Reflector.ForgeHooksClient_onDrawBlockHighlight, var5, var16, this.mc.objectMouseOver, 0, var16.getHeldItem(), partialTicks)) && !this.mc.gameSettings.hideGUI) {
        ...
        Sulfur.getInstance().getEventBus().post(new EventEntityRender(partialTicks));
        ...
        boolean handRendered = Reflector.callBoolean(Reflector.ForgeHooksClient_renderFirstPersonHand, this.mc.renderGlobal, partialTicks, pass);
        Sulfur.getInstance().getEventBus().post(new Render3DEvent(partialTicks));
        if (!handRendered && !Shaders.isShadowPass) {
            if (isShaders) {
            ...
            for (int var14 = 0; var14 < var13; ++var14) {
                ...
                if (var15.getY() <= var4.getY() + var5 && var15.getY() >= var4.getY() - var5 && var16.canSpawnLightningBolt() && var16.func_180626_a(var15) >= 0.15F) {
                    ...
                    if (var18.getMaterial() == Material.lava) {
                        this.mc.theWorld.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, (float) var15.getX() + var19, (double) ((float) var15.getY() + 0.1F) - var18.getBlockBoundsMinY(), (float) var15.getZ() + var20, 0.0D, 0.0D, 0.0D);
                        ...
                        this.mc.theWorld.spawnParticle(EnumParticleTypes.WATER_DROP, (float) var17.getX() + var19, (double) ((float) var17.getY() + 0.1F) + var18.getBlockBoundsMaxY(), (float) var17.getZ() + var20, 0.0D, 0.0D, 0.0D);
                    }
                }
            }
                    ...
                    if (var26.canSpawnLightningBolt() || var26.getEnableSnow()) {
                        ...
                        if (var28 != var29) {
                            ...
                            if (var4.getWorldChunkManager().getTemperatureAtHeight(var32, var27) >= 0.15F) {
                                ...
                                var9.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, var28, (double) ((float) var20 - var24) + 0.5D, 0.0F * var30, (float) var28 * var30 / 4.0F + var33 * var30);
                                var9.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, var28, (double) ((float) var20 + var24) + 0.5D, var30, (float) var28 * var30 / 4.0F + var33 * var30);
                                var9.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, var29, (double) ((float) var20 + var24) + 0.5D, var30, (float) var29 * var30 / 4.0F + var33 * var30);
                                var9.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, var29, (double) ((float) var20 - var24) + 0.5D, 0.0F * var30, (float) var29 * var30 / 4.0F + var33 * var30);
                                ...
                                var9.setTranslation(-var10, -var12, -var14);
                                var9.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, var28, (double) ((float) var20 - var24) + 0.5D, 0.0F * var30 + var44, (float) var28 * var30 / 4.0F + var33 * var30 + var35);
                                var9.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, var28, (double) ((float) var20 + var24) + 0.5D, 1.0F * var30 + var44, (float) var28 * var30 / 4.0F + var33 * var30 + var35);
                                var9.addVertexWithUV((double) ((float) var21 + var23) + 0.5D, var29, (double) ((float) var20 + var24) + 0.5D, 1.0F * var30 + var44, (float) var29 * var30 / 4.0F + var33 * var30 + var35);
                                var9.addVertexWithUV((double) ((float) var21 - var23) + 0.5D, var29, (double) ((float) var20 - var24) + 0.5D, 0.0F * var30 + var44, (float) var29 * var30 / 4.0F + var33 * var30 + var35);
                                var9.setTranslation(0.0D, 0.0D, 0.0D);
                            }
                        }
                    }
    ...
    private void updateFogColor(float partialTicks) {
        ...
        if (this.cloudFog) {
            ...
            fogYFactor = CustomColors.getUnderwaterColor(this.mc.theWorld, this.mc.getRenderViewEntity().posX, this.mc.getRenderViewEntity().posY + 1.0D, this.mc.getRenderViewEntity().posZ);
        ...
        if (Reflector.EntityViewRenderEvent_FogColors_Constructor.exists()) {
            Object event1 = Reflector.newInstance(Reflector.EntityViewRenderEvent_FogColors_Constructor, this, var3, var211, partialTicks, this.field_175080_Q, this.field_175082_R, this.field_175081_S);
    ...
    private void frameInit() {
        ...
        if (world != null) {
            ...
            if (Config.isNotify64BitJava()) {
                ...
                ChatComponentText msg2 = new ChatComponentText(I18n.format("of.message.java64Bit"));
