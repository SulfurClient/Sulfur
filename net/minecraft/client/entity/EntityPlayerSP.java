...
import gg.sulfur.client.impl.modules.movement.NoSlow;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void moveEntity(double x, double y, double z) {
        MovementEvent event = new MovementEvent(x, y, z);
    ...
    public void func_175161_p() {
        ...
        if (this.func_175160_A()) {
            ...
            Sulfur.getInstance().getEventBus().post(event);
            double var3 = event.getPosX() - this.field_175172_bI;
            double var5 = event.getPosY() - this.field_175166_bJ;
            double var7 = event.getPosZ() - this.field_175167_bK;
            double var9 = event.getRotationYaw() - this.field_175164_bL;
            double var11 = event.getRotationPitch() - this.field_175165_bM;
            boolean var13 = var3 * var3 + var5 * var5 + var7 * var7 > 9.0E-4D || this.field_175168_bP >= 20;
            boolean var14 = var9 != 0.0D || var11 != 0.0D;

            if (this.ridingEntity == null) {
                if (var13 && var14) {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(event.getPosX(), event.getPosY(), event.getPosZ(), event.getRotationYaw(), event.getRotationPitch(), event.isOnGround()));
                } else if (var13) {
            ...
            if (var13) {
                this.field_175172_bI = event.getPosX();
                this.field_175166_bJ = event.getPosY();
                this.field_175167_bK = event.getPosZ();
                this.field_175168_bP = 0;
            }

            if (var14) {
                this.field_175164_bL = event.getRotationYaw();
                this.field_175165_bM = event.getRotationPitch();
            }
        }
        Sulfur.getInstance().getEventBus().post(new UpdateEvent(false));
     ...
     */
    public void dropOneItem(boolean p_71040_1_) {
    ...
    public void sendChatMessage(String message) {
        ...
        Sulfur.getInstance().getEventBus().post(chatEvent);
        if (chatEvent.isCancelled())
            return;
        if (message.startsWith("#")) {
            ChatUtil.displayMessage("§aChat §7» §7" + Sulfur.getInstance().getUser() + ": §f" + message.replaceAll("#", ""));
            new Thread(() -> {
                Util.message("§aChat §7» §7" + Sulfur.getInstance().getUser() + ": §f" + message.replaceAll("#", ""), Channels.CHAT);
            }).start();
            return;
        }
        this.sendQueue.addToSendQueue(new C01PacketChatMessage(chatEvent.getRawMessage()));
    }
    ...
    public void jump() {
        ...
        Sulfur.getInstance().getEventBus().post(jumpEvent);
    ...
    public void onLivingUpdate() {
        if (this.sprintingTicksLeft > 0) {
            ...
            if (this.sprintingTicksLeft == 0) {
                //todo: undo please???? this.setSprinting(false);
            }
        }
        ...
        if (this.isUsingItem() && !this.isRiding() && !Sulfur.getInstance().getModuleManager().get(NoSlow.class).isToggled()) {
    ...
    public void damagePlayer() {
        ...
        for (int i = 0; i < (isPotionActive(Potion.jump) ? 122 : 49); ++i) {
            for (int length = normalValue.length, j = 0; j < length; ++j) {
                this.mc.getNetHandler().getNetworkManager().sendPacket(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, this.posY + normalValue[j], this.posZ, false));
            }
        }
    ...
    public void damagePlayerHypixelModeThoLolFuckingNiggerMadeThisMethodTooLongWhy() {
        if (!onGround) return;
        double[] normalValue = new double[]{0.062, 0.0};
        double[] hypixelValue = new double[]{0.422993999998688697815, 0.002140803780930446};
            for (int i = 0; i < (isPotionActive(Potion.jump) ? 15 : 8); ++i) {
                for (int length = hypixelValue.length, j = 0; j < length; ++j) {
                    this.mc.getNetHandler().getNetworkManager().sendPacket(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, this.posY + hypixelValue[j], this.posZ, false));
                }
            }
    }
}
