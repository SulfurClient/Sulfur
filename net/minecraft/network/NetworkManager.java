...
import gg.sulfur.client.api.proxy.MinecraftProxyHandler;
...
//            String commonTransformerDec = ReflectUtils.getField(null, "com.github.creeper123123321.viafabric.handler.CommonTransformer", "HANDLER_DECODER_NAME").toString();
            ...
            bootstrap.handler(new ChannelInitializer<OioSocketChannel>() {
                ...
                protected void initChannel(OioSocketChannel socketChannel) {
                    ...
                    if (Sulfur.getInstance().isViaVersionEnabled()) {
                        if (socketChannel instanceof SocketChannel && ViaMCP.getInstance().getVersion() != ViaMCP.PROTOCOL_VERSION) {
                            UserConnection user = new UserConnectionImpl(socketChannel, true);
                            new ProtocolPipelineImpl(user);
                            socketChannel.pipeline().addBefore("encoder", CommonTransformer.HANDLER_ENCODER_NAME, new VREncodeHandler(user)).addBefore("decoder", CommonTransformer.HANDLER_DECODER_NAME, new VRDecodeHandler(user));
                        }
                    }

                }
            });
            bootstrap.channelFactory(new MinecraftProxyHandler(Sulfur.getInstance().getProxy() == null ? Proxy.NO_PROXY : Sulfur.getInstance().getProxy()));
