...
public class BlockDoor extends Block {
    ...
    private void func_150011_b(int p_150011_1_) {
        ...
        if (var4) {
            ...
            /*
            Me and PC were making a joke about changing mc sources from alot of if else if else tyoo switch statements like
            YandereDev should have done
             */
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (this.blockMaterial == Material.iron) {
            ...
            if (var10.getBlock() != this) {
                ...
                worldIn.playAuxSFXAtEntity(playerIn, (Boolean) state.getValue(OPEN_PROP) ? 1003 : 1006, pos, 0);
                return true;
            }
        }
    }
    ...
    public IBlockState getStateFromMeta(int meta) {
        return (meta & 8) > 0 ? this.getDefaultState().withProperty(HALF_PROP, BlockDoor.EnumDoorHalf.UPPER).withProperty(HINGEPOSITION_PROP, (meta & 1) > 0 ? BlockDoor.EnumHingePosition.RIGHT : BlockDoor.EnumHingePosition.LEFT).withProperty(POWERED_PROP, (meta & 2) > 0) : this.getDefaultState().withProperty(HALF_PROP, BlockDoor.EnumDoorHalf.LOWER).withProperty(FACING_PROP, EnumFacing.getHorizontal(meta & 3).rotateYCCW()).withProperty(OPEN_PROP, (meta & 4) > 0);
    }
