...
public abstract class BlockLiquid extends Block {
    ...
    public AxisAlignedBB getCollisionBoundingBox(World worldIn, BlockPos pos, IBlockState state) {
        BlockCollisionEvent event = new BlockCollisionEvent(null, this, pos.getX(), pos.getY(), pos.getZ());
        return event.getAxisAlignedBB();
    }
    ...
    public int getMixedBrightnessForBlock(IBlockAccess worldIn, BlockPos pos) {
        ...
        return (Math.max(var5, var6)) | (Math.max(var7, var8)) << 16;
    }
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (this.blockMaterial == Material.water) {
            ...
            if (var11 > 0 && var11 < 8) {
                if (rand.nextInt(64) == 0) {
                    worldIn.playSound(var5 + 0.5D, var7 + 0.5D, var9 + 0.5D, "liquid.water", rand.nextFloat() * 0.25F + 0.75F, rand.nextFloat() + 0.5F, false);
                }
            } else if (rand.nextInt(10) == 0) {
                worldIn.spawnParticle(EnumParticleTypes.SUSPENDED, var5 + (double) rand.nextFloat(), var7 + (double) rand.nextFloat(), var9 + (double) rand.nextFloat(), 0.0D, 0.0D, 0.0D);
            }
        }
        ...
        if (this.blockMaterial == Material.lava && worldIn.getBlockState(pos.offsetUp()).getBlock().getMaterial() == Material.air && !worldIn.getBlockState(pos.offsetUp()).getBlock().isOpaqueCube()) {
            if (rand.nextInt(100) == 0) {
                ...
                worldIn.spawnParticle(EnumParticleTypes.LAVA, var18, var13, var15, 0.0D, 0.0D, 0.0D);
    ...
    protected void func_180688_d(World worldIn, BlockPos p_180688_2_) {
        ...
        for (int var9 = 0; var9 < 8; ++var9) {
            worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, var3 + Math.random(), var5 + 1.2D, var7 + Math.random(), 0.0D, 0.0D, 0.0D);
        }
    }
