...
import gg.sulfur.client.impl.events.StrafeEvent;
...
public abstract class Entity implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            double var17 = z;
            //todo: safewalk event?
            Scaffold scaffold = Sulfur.getInstance().getModuleManager().get(Scaffold.class);
            boolean var19 = this.onGround && this.isSneaking() && this instanceof EntityPlayer;
            boolean safewalk = scaffold != null && scaffold.safewalk.getValue() && scaffold.isToggled();
            ...
            EntityStepEvent entityStepEvent = new EntityStepEvent(stepHeight, true, this);
            if (this instanceof EntityPlayerSP) Sulfur.getInstance().getEventBus().post(entityStepEvent);
            if (entityStepEvent.getHeight() > 0.0F && !entityStepEvent.isCancelled() && var54 && (var13 != x || var17 != z)) {
                double var56 = x;
                double var25 = y;
                double var27 = z;
                if (var56 * var56 + var27 * var27 >= x * x + z * z) {
                    if (!entityStepEvent.isCancelled()) {
                        if (entityStepEvent.getHeight() > 0.0D && (var13 != x || var17 != z)) {
                            AxisAlignedBB var29 = getEntityBoundingBox();
                            func_174826_a(var21);
                            y = entityStepEvent.getHeight();
                            ...
                            for (Iterator var50 = var30.iterator(); var50
                            ...
                            if (var56 * var56 + var27 * var27 >= x * x + z * z) {
                                ...
                                if (this == Minecraft.getMinecraft().thePlayer)
                                    Sulfur.getInstance().getEventBus().post(new EntityStepEvent(1.0F + (float) y, false, this));
                            }
                        }
                    }
                }
            ...
            //boolean force = this instanceof EntityPlayerSP && Client.INSTANCE.getModuleManager().get(Phase.class).isToggled();
            this.onGround = /*force ||*/ (this.isCollidedVertically && var15 < 0.0D);
            this.isCollided = this.isCollidedHorizontally || this.isCollidedVertically;
            int var57 = MathHelper.floor_double(this.posX);
            int var58 = MathHelper.floor_double(this.posY - 0.0);
    ...
    protected void resetHeight() {
        ...
        for (var3 = 0; (float) var3 < 1.0F + this.width * 20.0F; ++var3) {
            ...
            this.worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (double) var4, var2 + 1.0F, this.posZ + (double) var5, this.motionX, this.motionY - (double) (this.rand.nextFloat() * 0.2F), this.motionZ);
        }

        for (var3 = 0; (float) var3 < 1.0F + this.width * 20.0F; ++var3) {
            var4 = (this.rand.nextFloat() * 2.0F - 1.0F) * this.width;
            var5 = (this.rand.nextFloat() * 2.0F - 1.0F) * this.width;
            this.worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, this.posX + (double) var4, var2 + 1.0F, this.posZ + (double) var5, this.motionX, this.motionY, this.motionZ);
        }
    }
    ...
    public void moveFlying(double strafe, double forward, double friction) {
        StrafeEvent event = new StrafeEvent(strafe, forward, friction, this.rotationYaw, this.rotationPitch, StrafeEvent.State.STRAFE);
        if (this instanceof EntityPlayerSP)
            Sulfur.getInstance().getEventBus().post(event);
        ...
        double var4 = strafe * strafe + forward * forward;
        ...
        if (var4 >= 1.0E-4F) {
            ...
            strafe *= var4;
            forward *= var4;
            double var5 = Math.sin(event.getYaw() * (float) Math.PI / 180.0F);
            double var6 = Math.cos(event.getYaw() * (float) Math.PI / 180.0F);
            if (silent) {
                double x = this.motionX + (strafe * var6 - forward * var5);
                double z = this.motionZ + (forward * var6 + strafe * var5);
                PacketUtil.sendPacketNoEvent(new C03PacketPlayer.C06PacketPlayerPosLook(this.posX + x, this.posY, this.posZ + z, event.getYaw(), event.getPitch(), this.onGround));
                var5 = MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F);
                var6 = MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F);
            }
            this.motionX += strafe * var6 - forward * var5;
            this.motionZ += forward * var6 + strafe * var5;
        } else if (silent) {
            PacketUtil.sendPacketNoEvent(new C03PacketPlayer(this.onGround));
        }
    }
    ...
    public void func_174828_a(BlockPos p_174828_1_, float p_174828_2_, float p_174828_3_) {
        this.setLocationAndAngles((double) p_174828_1_.getX() + 0.5D, p_174828_1_.getY(), (double) p_174828_1_.getZ() + 0.5D, p_174828_2_, p_174828_3_);
    }
    ...
    public boolean isEntityInsideOpaqueBlock() {
        if (this.noClip) {
            ...
            for (int var1 = 0; var1 < 8; ++var1) {
                double var2 = this.posX + (double) (((float) ((var1) % 2) - 0.5F) * this.width * 0.8F);
    ...
    public void func_180426_a(double p_180426_1_, double p_180426_3_, double p_180426_5_, float p_180426_7_, float p_180426_8_, int p_180426_9_, boolean p_180426_10_) {
        ...
        if (!var11.isEmpty()) {
            ...
            for (Object o : var11) {
                AxisAlignedBB var15 = (AxisAlignedBB) o;
