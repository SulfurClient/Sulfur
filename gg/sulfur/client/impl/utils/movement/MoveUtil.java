...
public class MoveUtil {
    ...
    public static void setMoveSpeed(final MovementEvent event, final double speed) {
        ...
        if (forward == 0.0 && strafe == 0.0) {
            ...
            event.setMotionX(forward * speed * -Math.sin(Math.toRadians(yaw)) + strafe * speed * Math.cos(Math.toRadians(yaw)));
            event.setMotionZ(forward * speed * Math.cos(Math.toRadians(yaw)) - strafe * speed * -Math.sin(Math.toRadians(yaw)));
        }
    }

    public static void TP(MovementEvent event, double speed, double y) {
        float yaw = mc.thePlayer.rotationYaw;
