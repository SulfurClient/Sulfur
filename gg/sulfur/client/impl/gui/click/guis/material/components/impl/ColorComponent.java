...
public class ColorComponent extends Component {
    ...
    public ColorComponent(ColorValue colorValue, float posX, float posY, float offsetX, float offsetY, float width, float height) {
        super(colorValue.getName(), posX, posY, offsetX, offsetY, width, height);
        this.colorValue = colorValue;
        float[] hsb = new float[3];
        final Color clr = colorValue.getValue();
    ...
    public void onDrawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (float i = 0; i < getWidth() - 70; i++) {
            ...
            RenderUtils.drawRect((getPosX() + getWidth() - 100) + i, getPosY() + 4.0f, 1, 4, color);
            if (mouseX == (int)(getPosX() + getWidth() - 100) + i) {
                if (pressedhue) {
                    colorValue.setValue(Color.getHSBColor(i / (getWidth() - 70), saturation, brightness));
                    pos = i;
                }
            }
        }
        RenderUtils.drawRect(getPosX() + getWidth() - 100 + pos, getPosY() + 3.5f, 1, 4.5f,  0xffffffff);
        RenderUtils.drawOutlinedRoundedRect(getPosX()+ getWidth() - 100.5f,getPosY() + 3.5f, getWidth() - 69,5,3,2,new Color(45, 45, 45, 255).getRGB());
    }
