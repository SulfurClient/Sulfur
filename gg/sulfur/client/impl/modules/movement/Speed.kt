 ...
 * @project Client
 */
class Speed(moduleData: ModuleData?) : Module(moduleData) {

    //Credit: FDPClient (the Reflection stuff)
    private val modes = ReflectUtils.getReflects("${this.javaClass.`package`.name}.speed", SpeedMode::class.java).map { it.newInstance() as SpeedMode }.sortedBy { it.modeName }
    val mode: SpeedMode get() = modes.find { modeValue.equals(it.modeName) } ?: throw NullPointerException() // this should not happen
